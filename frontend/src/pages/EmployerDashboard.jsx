              ...
              <li key={app._id} className="border-b dark:border-gray-700 py-3 flex justify-between items-center">
                <div>
                  <p className="font-semibold text-gray-800 dark:text-gray-200">
                    {app.applicant ? `${app.applicant.firstName} ${app.applicant.lastName}` : 'N/A'}
                  </p>
                  <p className="text-sm text-gray-600 dark:text-gray-400">
                    Applied for: <Link to={`/jobs/${app.job?._id}`} className="text-blue-600 hover:underline">{app.job?.title || 'N/A'}</Link>
                  </p>
                </div>
                <span className="text-sm text-gray-500 dark:text-gray-400">{new Date(app.date).toLocaleDateString()}</span>
              </li>
            ))}
          </ul>
        ) : (
