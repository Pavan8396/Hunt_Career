...
const UserProfilePage = () => {
  ...
  if (loading) {
    return <div className="p-4 max-w-4xl mx-auto text-center ">Loading profile...</div>;
  }

  return (
    <div className="p-4 sm:p-6 lg:p-8 bg-gray-50  min-h-screen">
      <div className="max-w-screen-lg mx-auto">
        <h1 className="text-3xl sm:text-4xl font-bold text-gray-800  mb-8">Your Profile</h1>
            ...
            <div className="bg-white  p-6 rounded-xl shadow-lg">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <input type="text" name="firstName" id="firstName" value={profile.firstName} onChange={(e) => handleChange(e)} className="mt-1 block w-full px-3 py-2 bg-gray-50  border border-gray-300  rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm text-gray-800 " />
                  {errors.firstName && <p className="text-red-500 text-xs mt-1">{errors.firstName}</p>}
                </div>
                <div>
                  <label htmlFor="lastName" className="block text-sm font-medium text-gray-700 ">Last Name</label>
                  <input type="text" name="lastName" id="lastName" value={profile.lastName} onChange={(e) => handleChange(e)} className="mt-1 block w-full px-3 py-2 bg-gray-50  border border-gray-300  rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm text-gray-800 " />
                  {errors.lastName && <p className="text-red-500 text-xs mt-1">{errors.lastName}</p>}
                </div>
                <div>
                  <label htmlFor="email" className="block text-sm font-medium text-gray-700 ">Email</label>
                  <input type="email" name="email" id="email" value={profile.email} readOnly className="mt-1 block w-full px-3 py-2 bg-gray-200  border border-gray-300  rounded-md shadow-sm sm:text-sm text-gray-500  cursor-not-allowed" />
                </div>
                <div>
                  <label htmlFor="phoneNumber" className="block text-sm font-medium text-gray-700 ">Phone Number</label>
                  <input type="tel" name="phoneNumber" id="phoneNumber" value={profile.phoneNumber} onChange={(e) => handleChange(e)} className="mt-1 block w-full px-3 py-2 bg-gray-50  border border-gray-300  rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm text-gray-800 " />
                  ...
                  profile.workExperience.map((exp, index) => (
                    <div key={index} className="p-4 border border-gray-200  rounded-lg space-y-4 bg-gray-50 ">
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                          <input name="title" value={exp.title} onChange={(e) => handleChange(e, index, 'workExperience')} placeholder="Job Title" className="block w-full px-3 py-2 bg-white  border border-gray-300  rounded-md sm:text-sm text-gray-800 " />
                          {errors.workExperience?.[index]?.title && <p className="text-red-500 text-xs mt-1">{errors.workExperience[index].title}</p>}
                        </div>
                        <div>
                          <input name="company" value={exp.company} onChange={(e) => handleChange(e, index, 'workExperience')} placeholder="Company" className="block w-full px-3 py-2 bg-white  border border-gray-300  rounded-md sm:text-sm text-gray-800 " />
                          {errors.workExperience?.[index]?.company && <p className="text-red-500 text-xs mt-1">{errors.workExperience[index].company}</p>}
                        </div>
                        <input name="location" value={exp.location} onChange={(e) => handleChange(e, index, 'workExperience')} placeholder="Location" className="block w-full px-3 py-2 bg-white  border border-gray-300  rounded-md sm:text-sm text-gray-800 " />
                        <input name="startDate" type="date" value={exp.startDate ? new Date(exp.startDate).toISOString().split('T')[0] : ''} onChange={(e) => handleChange(e, index, 'workExperience')} placeholder="Start Date" className="block w-full px-3 py-2 bg-white  border border-gray-300  rounded-md sm:text-sm text-gray-800 " />
                        <input name="endDate" type="date" disabled={exp.present} value={exp.endDate ? new Date(exp.endDate).toISOString().split('T')[0] : ''} onChange={(e) => handleChange(e, index, 'workExperience')} placeholder="End Date" className="block w-full px-3 py-2 bg-white  border border-gray-300  rounded-md sm:text-sm text-gray-800  disabled:bg-gray-200 " />
                        <div className="flex items-center">
                          ...
                          <label htmlFor={`present-${index}`} className="ml-2 block text-sm text-gray-900 ">I currently work here</label>
                        </div>
                      </div>
                      <div className="flex items-center gap-4">
                        <textarea name="description" value={exp.description} onChange={(e) => handleChange(e, index, 'workExperience')} placeholder="Description" className="block w-full px-3 py-2 bg-white  border border-gray-300  rounded-md sm:text-sm text-gray-800 "></textarea>
                        <button type="button" onClick={() => removeSectionItem(index, 'workExperience')} className="text-red-600 hover:text-red-800  " title="Remove work experience">
                 ...
                 {profile.education.length > 0 ? (
                  profile.education.map((edu, index) => (
                    <div key={index} className="p-4 border border-gray-200  rounded-lg space-y-4 bg-gray-50 ">
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                          <input name="school" value={edu.school} onChange={(e) => handleChange(e, index, 'education')} placeholder="School" className="block w-full px-3 py-2 bg-white  border border-gray-300  rounded-md sm:text-sm text-gray-800 " />
                          {errors.education?.[index]?.school && <p className="text-red-500 text-xs mt-1">{errors.education[index].school}</p>}
                        </div>
                        <div>
                          <input name="degree" value={edu.degree} onChange={(e) => handleChange(e, index, 'education')} placeholder="Degree" className="block w-full px-3 py-2 bg-white  border border-gray-300  rounded-md sm:text-sm text-gray-800 " />
                          {errors.education?.[index]?.degree && <p className="text-red-500 text-xs mt-1">{errors.education[index].degree}</p>}
                        </div>
                        <input name="fieldOfStudy" value={edu.fieldOfStudy} onChange={(e) => handleChange(e, index, 'education')} placeholder="Field of Study" className="block w-full px-3 py-2 bg-white  border border-gray-300  rounded-md sm:text-sm text-gray-800 " />
                        <input name="startDate" type="date" value={edu.startDate ? new Date(edu.startDate).toISOString().split('T')[0] : ''} onChange={(e) => handleChange(e, index, 'education')} placeholder="Start Date" className="block w-full px-3 py-2 bg-white  border border-gray-300  rounded-md sm:text-sm text-gray-800 " />
                        <input name="endDate" type="date" value={edu.endDate ? new Date(edu.endDate).toISOString().split('T')[0] : ''} onChange={(e) => handleChange(e, index, 'education')} placeholder="End Date" className="block w-full px-3 py-2 bg-white  border border-gray-300  rounded-md sm:text-sm text-gray-800 " />
                      </div>
                      <div className="flex justify-end">
                        <button type="button" onClick={() => removeSectionItem(index, 'education')} className="text-red-600 hover:text-red-800  " title="Remove education">
                          <TrashIcon className="h-5 w-5" />
                        </button>
                      </div>
                    </div>
                ...
                {profile.skills.length > 0 ? (
                  profile.skills.map((skill, index) => (
                    <div key={index} className="flex items-center gap-2">
                      <input value={skill} onChange={(e) => handleListChange(e, index, 'skills')} placeholder="e.g., React" className="block w-full px-3 py-2 bg-gray-50  border border-gray-300  rounded-md sm:text-sm text-gray-800 " />
                      <button type="button" onClick={() => removeListItem(index, 'skills')} className="text-red-600 hover:text-red-800  " title="Remove skill"><TrashIcon className="h-5 w-5" /></button>
                    </div>
                  ))
                ) : (
                  <div className="text-center text-gray-500  py-6 border-2 border-dashed border-gray-300  rounded-lg">
              ...
              <div className="space-y-2">
                {profile.portfolioLinks.length > 0 ? (
                  profile.portfolioLinks.map((link, index) => (
                    <div key={index} className="flex items-center gap-2">
                      <input value={link} onChange={(e) => handleListChange(e, index, 'portfolioLinks')} placeholder="https://github.com/johndoe" className="block w-full px-3 py-2 bg-gray-50  border border-gray-300  rounded-md sm:text-sm text-gray-800 " />
                      <button type="button" onClick={() => removeListItem(index, 'portfolioLinks')} className="text-red-600 hover:text-red-800  " title="Remove link"><TrashIcon className="h-5 w-5" /></button>
